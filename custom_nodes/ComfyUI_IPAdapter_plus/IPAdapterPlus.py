...
import ast
import json
from .mask_proc import roi_mask_proc
...
def set_model_patch_replace(model, patch_kwargs, key):
    ...
    if "patches_replace" not in to:
        #print("patches_replace_1")
        to["patches_replace"] = {}
    else:
        #print("patches_replace_2")
        to["patches_replace"] = to["patches_replace"].copy()

    if "attn2" not in to["patches_replace"]:
        #print("patches_replace_3")
        to["patches_replace"]["attn2"] = {}
    else:
        #print("patches_replace_4")
        to["patches_replace"]["attn2"] = to["patches_replace"]["attn2"].copy()
    
    if key not in to["patches_replace"]["attn2"]:
        #print("patches_replace_5")
        to["patches_replace"]["attn2"][key] = Attn2Replace(ipadapter_attention, **patch_kwargs)
        model.model_options["transformer_options"] = to
    else:
        #print("patches_replace_6")
...
def ipadapter_execute(model,
    ...
    if roi_mask_attrs != {}:
        whs, mask_rp_res = roi_mask_proc(roi_mask_attrs)
        ...
        patch_kwargs = {
            ...
            "roi_mask_dict": {
                "whs": whs,
                "mask_rp_res": mask_rp_res,
                "ip_rp_weight": roi_mask_attrs['merge_weight'],
            }
        }
    else:
        patch_kwargs = {
...
class IPAdapterAdvanced:
    def __init__(self):
        print("IPAdapterAdvanced_init")
        self.unfold_batch = False

    @classmethod
    def INPUT_TYPES(s):
        print("IPAdapterAdvanced input")
        return {
            "required": {
                "model": ("MODEL", ),
                "ipadapter": ("IPADAPTER", ),
    ...
    def apply_ipadapter(
        ...
        print("IPAdapterAdvanced_apply_ipadapter")
        is_sdxl = isinstance(model.model, (comfy.model_base.SDXL, comfy.model_base.SDXLRefiner, comfy.model_base.SDXL_instructpix2pix))

        print("ipadapter: {}, ipadapter_params: {}".format(ipadapter, ipadapter_params))
        ...
        for i in range(len(image)):
            ...
            print("ipa_args: {}".format(ipa_args))
...
class IPAdapterROIAdvanced:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "attn_mask": ("MASK",),
                "clip_vision": ("CLIP_VISION",),
                "roi_mask_str": ("STRING",{"multiline": True,  "default": "{}"}),
            }
        }
    ...
    def apply_ipadapter(
        ...
        if roi_mask_str != '{}':
            roi_mask_dict = self.load_roi_mask(roi_mask_str) 
            roi_mask_attrs = {
                "height": roi_mask_dict.get('gen_height', 1024) ,
                "width": roi_mask_dict.get('gen_width', 576) ,
                "roi_mask": roi_mask_dict.get('rp_instance', ''),
                "merge_weight": roi_mask_dict.get('ip_rp_weight', 0.95),
                "rp_split_ratio": roi_mask_dict.get('rp_split_ratio', 0.5),
                "rp_bbox": roi_mask_dict.get('rp_bbox', []),
            }    
...
NODE_CLASS_MAPPINGS = {
    ...
    "IPAdapterROIAdvanced": IPAdapterROIAdvanced,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "IPAdapterROIAdvanced": "IPAdapter ROI Advanced",
